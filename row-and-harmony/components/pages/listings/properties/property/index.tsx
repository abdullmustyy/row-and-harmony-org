...
interface IPropertyPageProps
    extends Omit<React.ComponentProps<"main">, "property"> {
    ...
    const showcaseImages = useMemo(
        () =>
            property.images?.showcase
                ? property.images.showcase
                      .slice(0, 4)
                      .map((showcase) => urlFor(showcase).url())
                : [],
        [property.images?.showcase],
    );
        ...
        <main className="relative isolate mt-nav">
            <section className="flex flex-col md:px-8 px-4 py-[5vh]">
                <div className="flex items-center md:gap-8 gap-4 pl-2">
                    ...
                    <h3 className="md:text-fluid-xl text-fluid-lg font-primary font-medium">
                        {property.title}
                    </h3>
                    <Button variant="ghost" className="ml-auto">
                        <Share />
                        <span className="md:inline hidden underline underline-offset-2">
                            Share
                        </span>
                    </Button>
                </div>
                ...
                <div className="flex md:flex-row flex-col items-start lg:gap-20 gap-8">
                    ...
                    <aside className="md:w-1/3 w-full grid gap-6">
                        <div>
                            ...
                            <p>
                                For {property.type}s inquiries, please reach out
